OT_economicData = [
    [[7270.84,8391.29,0],"Diesel Power Plant","","OT_Plastic"],
    [[8939.47,6627.97,0],"Cocoa Plantations"],
    [[9456.82,7552.04,0],"Lami Lumberyard","","OT_Wood"],
    [[8389.24,10229.8,0],"Tanoa Sugar Company","OT_Sugarcane","OT_Sugar"],
    [[10945.1,7860.1,0],"Kotomo Banana Plantation","","OT_Fertilizer"],
    [[11761.2,6949.91,0],"Sawmill","OT_Wood","OT_Lumber"],
    [[5511.58,11888.2,0],"Saint-Julien Steel Yard","","OT_Steel"],
    [[8878.1,11889.1,0],"Galili Lumberyard","","OT_Wood"],
    [[12507.4,7820.99,0],"Ouméré Banana Plantations","","OT_Fertilizer"],
    [[8526.85,12467.3,0],"Roadhouse"],
    [[11921.1,10206.5,0],"Red Spring Surface Mine","","OT_Steel"],
    [[12383,14135.3,0],"Ravi-ta Island Resort"],
    [[4687.63,3590.75,0],"Katkoula Sugar Plantations","","OT_Fertilizer"],
    [[8026.38,8946.16,0],"Lifou Sugar Plantations","","OT_Sugarcane"],
    [[12445.6,3982.46,0],"Saioko Steel Yard","","OT_Steel"]
];
OT_factoryPos = [4226.49,8370.82,0];
OT_factoryVehicleSpawn = [4265.64,8400.65,0];
OT_factoryVehicleDir = 67;
OT_connectedRegions = [
    ["island_1","island_2"],
    ["island_2","island_1"],
    ["island_3","island_4"],
    ["island_4","island_3"]
];
